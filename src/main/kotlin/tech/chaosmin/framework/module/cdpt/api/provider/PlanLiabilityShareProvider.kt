...
open class PlanLiabilityShareProvider(
    ...
    override fun page(request: HttpServletRequest): RestResult<IPage<PlanLiabilityResp>> {
        ...
        return RestResultExt.successRestResult(
            page.convert(PlanLiabilityMapper.INSTANCE::convert2Entity).convert {
                PlanLiabilityConvert.INSTANCE.convert2Resp(it!!)
            })
    }

    override fun save(req: PlanLiabilityReq): RestResult<PlanLiabilityResp> {
        val planLiability = PlanLiabilityConvert.INSTANCE.convert2Entity(req).save()
        val result = modifyPlanLiabilityHandler.operate(planLiability)
        return RestResultExt.mapper<PlanLiabilityResp>(result).convert {
            PlanLiabilityConvert.INSTANCE.convert2Resp(result.data ?: PlanLiabilityEntity())
        }
    }

    override fun update(id: Long, req: PlanLiabilityReq): RestResult<PlanLiabilityResp> {
        val planLiability = PlanLiabilityConvert.INSTANCE.convert2Entity(req).update(id)
        val result = modifyPlanLiabilityHandler.operate(planLiability)
        return RestResultExt.mapper<PlanLiabilityResp>(result).convert {
            PlanLiabilityConvert.INSTANCE.convert2Resp(result.data ?: PlanLiabilityEntity())
        }
    ...
    override fun delete(id: Long): RestResult<PlanLiabilityResp> {
        val planLiability = PlanLiabilityEntity(id).remove()
        val result = modifyPlanLiabilityHandler.operate(planLiability)
        return RestResultExt.mapper<PlanLiabilityResp>(result).convert {
            PlanLiabilityConvert.INSTANCE.convert2Resp(result.data ?: PlanLiabilityEntity())
        }
